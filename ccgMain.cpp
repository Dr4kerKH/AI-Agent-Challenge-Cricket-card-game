...
int main()
{
   ...
   int i, j, k, numAWon, numBWon, numDrawn, numDraws[numAgents],
       numLosses[numAgents], numTied, numTies[numAgents],
       numWins[numAgents], order[numAgents], pointsAsA[numAgents],
       pointsAsB[numAgents], temp;
   MatchState match;

   srandom(time(0));

   cout << "Agent Challenge B: Cricket card game\n"
        << "Iteration results\n";
      ...
      for (j = 0; j < numAgents; j += 1)
      {
         if (i != j)
         {
            ...
            cout << "A (" << agentStr[i] << ") wins " << numAWon << ", B ("
                 << agentStr[j] << ") wins " << numBWon << ", draws "
                 << numDrawn << ", ties " << numTied << "\n";
         }
      }
   }
   ...
   cout << "\n\n"
        << "Overall standings:                                    points         batting statistics      bowling statistics\n"
        << "                    points     W     L     D     T  as A  as B      R/W     R/hB    B/W     R/W     R/hB    B/W\n";
   cout << fixed;
   for (i = 0; i < numAgents; i += 1)
   {
      cout << setw(20) << left << agentStr[order[i]]
           ...
           << " " << setprecision(2) << setw(7) << right << static_cast<double>(battingStats[order[i]].balls) / battingStats[order[i]].wickets
           << " " << setprecision(2) << setw(7) << right << static_cast<double>(bowlingStats[order[i]].runs) / bowlingStats[order[i]].wickets
           << " " << setprecision(2) << setw(7) << right << 100 * static_cast<double>(bowlingStats[order[i]].runs) / bowlingStats[order[i]].balls
