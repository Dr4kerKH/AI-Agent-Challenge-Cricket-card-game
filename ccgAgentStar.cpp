...
int ccgAgentStar(Hand hand, Card lastBowledCard, bool isBatting, const MatchState &match)
{
	Card card;
	...
	if (isBatting) {
		...
		if (highNum <= lastCard) {
			for (whichCard = 0; whichCard < numCardsPerHand; whichCard += 1) {
				...
				if (card.getSuit() == lastSuit)
					quality[whichCard] += (numCardsPerSuit - card.getNumber()) * 220;
			}	
		}
		...
		if (diffSuit == 6) {
			for (whichCard = 0; whichCard < numCardsPerHand; whichCard += 1) {
				...
				// ++Favour conditions
				if (numDiff == 0)
					quality[whichCard] += 750;
				if (numDiff >= 0 && numDiff < 2)
					quality[whichCard] += 650;
				if (numDiff < 0)
					quality[whichCard] -= 1150;
				if (numDiff > 3)
					quality[whichCard] -= 450;
			}
		}
		
		// Input condition where if every cards are != suit as well as all the 
		// cards are lower than bowling card, thus no matter card plays, wicket
		// is lost. Minimize wasted big card when no other options.
	}
	// Action: Bowling
	else {
